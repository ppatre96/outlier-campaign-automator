-- Pass B': Pooled funnel with PDR added. No Multimango, no Squads.
-- PDR source: VIEW.CBPR__COMBINED_PDR_BY_USER
--   - Grain: (USER_ID, PROJECT_ID)
--   - Column: COMBINED_PDR_RATE_AVG  (combined QC+CBA PDR rate, 0-1, lower is better = fewer defects)
...
cbpr_joined AS (
    ...
    LEFT JOIN VIEW.CBPR__COMBINED_PDR_BY_USER p
        ON cc.USER_ID = p.USER_ID AND cc.project_id = p.PROJECT_ID
),
retention_30d AS (
    SELECT
        channel_bucket,
...
main_agg AS (
    SELECT
        ...
        COUNT(CASE WHEN COMBINED_PDR_RATE_AVG IS NOT NULL THEN 1 END) AS pdr_non_null,
        AVG(COMBINED_PDR_RATE_AVG) AS avg_pdr_rate
    FROM cbpr_joined
    GROUP BY channel_bucket
)
SELECT
...
WHERE m.channel_bucket IN ('MWF','LATAM','Joveo','LinkedIn','Organic')
